...
def create_ledger_on_fee_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        # Log the error but don't prevent the fee from being saved
        print(f"Error creating ledger entry for fee {instance.id}: {str(e)}")
...
def create_ledger_on_repayment_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        # Log the error but don't prevent the repayment from being saved
        print(f"Error creating ledger entry for repayment {instance.id}: {str(e)}")
